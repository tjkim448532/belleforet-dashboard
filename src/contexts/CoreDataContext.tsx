...
export const CoreDataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const fetchCoreData = async () => {
      ...
      try {
        const [coreRes, summaryRes, matrixRes] = await Promise.all([
          secureFetcher(`${API_BASE}/api/v3/dashboard/core?targetDate=${targetDate}`),
          secureFetcher(`${API_BASE}/api/v3/dashboard/revenue-summary?date=${targetDate}`),
          secureFetcher(`${API_BASE}/api/dashboard/matrix?date=${targetDate}`)
        ]);

        setState({
          core: coreRes.data || coreRes,
          summary: summaryRes.data || summaryRes,
          matrix: matrixRes.data || matrixRes,
          isLoading: false,
          error: null
        });
      } catch (error) {
