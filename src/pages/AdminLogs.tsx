    ...
    <div className="w-full max-w-5xl mx-auto bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
      ...
      <div className="bg-slate-900 p-6 text-white flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold mb-1 flex items-center gap-2 break-keep whitespace-nowrap">
              임직원 접속 및 보안 로그 <ShieldCheck size={18} className="text-brand-mint shrink-0" />
            </h1>
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-red-500/20 hover:bg-red-500/30 text-red-300 px-4 py-2 rounded-xl transition-colors font-semibold text-xs border border-red-500/30 whitespace-nowrap shrink-0"
        >
      ...
      {/* Info Banner */}
      <div className="bg-slate-50 border-b border-slate-200 p-4 text-xs text-slate-600 flex items-center justify-between whitespace-nowrap">
        <div className="flex items-center gap-2 whitespace-nowrap">
          <Users size={16} className="text-brand-mint shrink-0" />
          <span>최근 100건의 임직원 접속 로그를 실시간으로 기록합니다.</span>
        </div>
        <span className="font-bold text-slate-800 whitespace-nowrap">총 {loginLogs.length}건 기록됨</span>
      </div>
        ...
        {loadingLogin ? (
          ...
          <table className="w-full text-left text-sm text-slate-600 whitespace-nowrap min-w-[500px]">
            <thead className="bg-slate-50 sticky top-0 border-b border-slate-200 text-slate-500 text-xs font-semibold">
              <tr>
                <th className="p-4 w-20 text-center whitespace-nowrap">번호</th>
                <th className="p-4 whitespace-nowrap">접속 임직원 이메일 계정</th>
                <th className="p-4 w-64 text-right whitespace-nowrap">접속 일시 (KST)</th>
              </tr>
            </thead>
