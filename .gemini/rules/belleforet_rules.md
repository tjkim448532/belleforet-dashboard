...
## 7. 🏷️ 벨포레 43개 공식 영업장_항목명 ➔ 6대 시스템 그룹 매핑 SSOT (The Definitive Mapping)
임의 분류를 엄격히 금지하며, 반드시 아래 43개 원천 항목 및 분류 매핑 체계를 100% 준수해야 합니다:
...
- **🎢 레저본부 (LEISURE)**: `익스트림 루지`, `마운틴카트`, `사계절썰매장`, `마리나 클럽`, `놀이동산`, `벨포레 목장`, `벨포레 목장(체험)`, `얼룩말카페` (※ 얼룩말카페는 레저본부 귀속), `디노 시네마`, `미디어아트센터`, `미디어-뮤지엄카페`, `미디어-기프트샵`, `썸머랜드`, `원더풀`, `미니골프`, `회전그네`
