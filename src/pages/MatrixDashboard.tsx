...
export default function MatrixDashboard() {
  ...
  categoriesOrder.forEach(code => {
    ...
    const detailRows = allRows.filter(r => !r.isSubtotal && r.shop_name !== categoryLabels[code] && !r.shop_name.includes('- Posting'));
        ...
        <table className="w-full text-sm text-right whitespace-nowrap">
          ...
          <tbody className="divide-y divide-slate-200 text-slate-700">
            {categoriesOrder.map(category => {
              ...
              const rows = allRows.filter(r => !r.isSubtotal && r.shop_name !== categoryLabels[category] && !r.shop_name.includes('- Posting'));
