    ...
    try {
      ...
      if (payload && (payload.totalUniqueCustomers !== undefined || (Array.isArray(payload.bundleClusters) && payload.bundleClusters.length > 0) || (Array.isArray(payload.data) && payload.data.length > 0))) {
        if (Array.isArray(payload.bundleClusters) && payload.bundleClusters.length > 0) {
          ...
          // 🚨 [Pure Consumer] 가짜 데이터 창조(Hallucination) 섀도우 로직 전면 철거
          setBundleData([]);
          setApiMeta({
            totalUniqueCustomers: 0,
            multiFacilityRatioPct: 0,
            totalSales: 0,
            multiFacilityCustomers: 0,
            singleFacilityArpu: 0,
            multiFacilityArpu: 0,
            arpuLiftMultiplier: 0
          });
        }
    ...
    <div className="p-6 lg:p-10 max-w-[1600px] mx-auto min-h-screen bg-slate-50/50">
      ...
      <div className="bg-gradient-to-r from-cyan-950 via-slate-900 to-indigo-950 rounded-[32px] p-8 text-white mb-8 shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col xl:flex-row xl:items-center justify-between gap-6">
          <div>
            ...
            <h1 className="text-3xl lg:text-4xl font-bold tracking-tight mt-1 flex items-center gap-3 flex-wrap break-keep">
              <CreditCard className="text-cyan-400 shrink-0" size={32} />
              <span className="break-keep">고객 결제 동선 기반 이용 묶음(Bundle) 분석</span>
            </h1>
            <p className="text-cyan-100 mt-2 text-sm lg:text-base font-normal max-w-3xl leading-relaxed break-keep">
            ...
            <div className="flex items-center gap-3 mt-6 pt-4 border-t border-white/10 flex-wrap">
              <NavLink 
                ...
                className={({ isActive }) => 
                  `px-4 py-2 rounded-xl text-xs font-semibold transition-all flex items-center gap-1.5 whitespace-nowrap ${
                    isActive 
              ...
              >
                <Sparkles size={14} /> <span className="whitespace-nowrap">1. 객실 세그먼트/채널 시너지 분석</span>
              </NavLink>
              ...
              >
                <Zap size={14} /> <span className="whitespace-nowrap">2. 앵커시설 순수 인과 & CAPA 분석</span>
              </NavLink>
              
              <NavLink 
                to="/synergy/bundles" 
                className="px-4 py-2 rounded-xl text-xs font-semibold transition-all flex items-center gap-1.5 bg-cyan-500 text-white shadow-lg ring-2 ring-cyan-400/30 whitespace-nowrap"
              >
                <CreditCard size={14} /> <span className="whitespace-nowrap">3. 고객 결제 묶음(Bundle) 분석</span>
              </NavLink>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-[24px] p-4 lg:p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 mb-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-cyan-50 text-cyan-700 rounded-xl shrink-0">
              <Calendar size={20} />
            </div>
            <div>
              <h2 className="text-base font-bold text-slate-800 break-keep">조회 기간 설정 및 묶음 동기화</h2>
              <p className="text-xs text-slate-400 break-keep">선택한 기간 동안의 카드 승인 전표를 전수 취합하여 클러스터를 산출합니다.</p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              <button onClick={() => applyPreset('TODAY')} className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 rounded-lg text-xs text-slate-700 font-medium whitespace-nowrap">오늘</button>
              <button onClick={() => applyPreset('WEEK')} className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 rounded-lg text-xs text-slate-700 font-medium whitespace-nowrap">최근 7일</button>
              <button onClick={() => applyPreset('MTD')} className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 rounded-lg text-xs text-slate-700 font-medium whitespace-nowrap">금월 (1일~오늘)</button>
              <button onClick={() => applyPreset('H1')} className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 rounded-lg text-xs text-slate-700 font-medium whitespace-nowrap">상반기</button>
              <button onClick={() => applyPreset('YTD')} className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 rounded-lg text-xs text-slate-700 font-medium whitespace-nowrap">연누계 (YTD)</button>
            </div>
            ...
            <div className="flex items-center gap-1.5">
              <input 
                ...
                className="bg-slate-50 border border-slate-200 text-slate-800 text-xs rounded-xl px-3 py-1.5 outline-none focus:border-cyan-500 font-medium"
              />
              {isRangeMode && (
                <>
            ...
            <button
              ...
              disabled={loading}
              className="px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl text-xs font-bold transition-all shadow-xs flex items-center justify-center gap-1 whitespace-nowrap shrink-0"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs font-bold text-cyan-800 bg-cyan-100 px-2.5 py-0.5 rounded-full whitespace-nowrap">
                {totalDays}일간 집계
              </span>
            </div>
            <div className="text-3xl font-black text-slate-900 my-1 tabular-nums whitespace-nowrap">
              {kpiStats.totalCustomers.toLocaleString()} <span className="text-sm font-normal text-slate-500">명</span>
            </div>
          </div>
          <p className="text-xs text-slate-500 mt-3 pt-2 border-t border-slate-100 break-keep">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs font-bold text-indigo-800 bg-indigo-100 px-2.5 py-0.5 rounded-full whitespace-nowrap">
                시너지 핵심
              </span>
            </div>
            <div className="text-3xl font-black text-indigo-600 my-1 tabular-nums whitespace-nowrap">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden flex flex-col justify-between">
          <div>
            ...
            <div className="text-xs font-bold text-amber-600 mt-1 tabular-nums whitespace-nowrap">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden flex flex-col justify-between">
          <div>
            ...
            <div className="text-3xl font-black text-emerald-600 my-1 tabular-nums whitespace-nowrap">
              {kpiStats.arpuLiftMultiplier ? `+${kpiStats.arpuLiftMultiplier}x` : '-'} <span className="text-sm font-normal text-slate-500">지출 증대</span>
            </div>
          </div>
          <p className="text-xs text-slate-500 mt-3 pt-2 border-t border-slate-100 tabular-nums break-keep">
      ...
      <div className="bg-white rounded-[32px] p-6 lg:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 mb-8">
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left text-sm whitespace-nowrap min-w-[950px]">
            <thead>
              <tr className="border-b border-slate-100 text-xs font-semibold text-slate-400 uppercase tracking-wider bg-slate-50/50">
                <th className="py-3.5 px-6 rounded-l-xl whitespace-nowrap">순위</th>
                <th className="py-3.5 px-6 whitespace-nowrap">고객 이용 묶음명</th>
                <th className="py-3.5 px-6 whitespace-nowrap">포함 영업장 리스트</th>
                <th className="py-3.5 px-6 text-right whitespace-nowrap">이용 고객수 (명)</th>
                <th className="py-3.5 px-6 text-right whitespace-nowrap">전체 비중 (%)</th>
                <th className="py-3.5 px-6 text-right whitespace-nowrap">묶음 총 매출액 (원)</th>
                <th className="py-3.5 px-6 text-right rounded-r-xl whitespace-nowrap">1인당 평균 결제액</th>
              </tr>
            </thead>
              ...
              {filteredBundles.length > 0 ? (
                filteredBundles.map((item, idx) => (
                  <tr key={idx} className="hover:bg-cyan-50/30 transition-colors">
                    <td className="py-4 px-6 font-bold text-slate-500 tabular-nums whitespace-nowrap">#{idx + 1}</td>
                    <td className="py-4 px-6 font-semibold text-slate-800 whitespace-nowrap">{formatBundleTitle(item)}</td>
                    <td className="py-4 px-6 text-xs text-slate-600">
                      <div className="flex items-center gap-1 flex-nowrap overflow-x-auto max-w-[360px]">
                        {item.storeList?.map((s, sIdx) => (
                          <span key={sIdx} className="bg-slate-100 text-slate-700 px-2 py-0.5 rounded text-xs font-medium whitespace-nowrap shrink-0">
                            {cleanStoreName(s)}
                          </span>
                        ))}
                      </div>
                    </td>
                    <td className="py-4 px-6 text-right font-medium text-slate-800 tabular-nums whitespace-nowrap">{item.customerCount.toLocaleString()}명</td>
                    <td className="py-4 px-6 text-right font-semibold text-cyan-700 tabular-nums whitespace-nowrap">{item.ratioPct}%</td>
                    <td className="py-4 px-6 text-right font-bold text-slate-900 tabular-nums whitespace-nowrap">{formatCurrency(item.totalSales)}원</td>
                    <td className="py-4 px-6 text-right font-bold text-indigo-700 tabular-nums whitespace-nowrap">{formatCurrency(item.avgSpendPerCustomer)}원/인</td>
                  </tr>
                ))
              ) : (
                <tr>
